...
<head>
    ...
    <script type="text/javascript">
        ...
        jQuery(function ($) {
            ...
            function load1() {
                $.ajax({
                    url: "/clue/clueActivities.json?clueId=${param.clueId}",
                    success(data) {
                        var html = "";
                        $(data).each(function () {
                            html += '<tr>';
                            html += '    <td>'+this.activity.name+'</td>';
                            html += '    <td>'+this.activity.startDate+'</td>';
                            html += '    <td>'+this.activity.endDate+'</td>';
                            html += '    <td>'+this.activity.owner+'</td>';
                            html += '    <td><a unbind="'+this.id+'" href="javascript:;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
                            html += '</tr>';
                        })
                        $("#dataBody").html(html);
            ...
            $("#dataBody").on("click", "a[unbind]", function () {
                $.ajax({
                    url: "/clue/unbind.do?id=" + $(this).attr("unbind"),
                    type: "delete",
                    success(data) {
                        if (data.success) {
                            load1();
